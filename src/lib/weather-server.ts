...
import { OpenWeatherMapResponse } from './weather';
  ...
  if (API_KEY) {
    try {
      ...
      const response = await fetch(
        ...
        { 
          next: { revalidate: 7200 } // Cache for 120 minutes at Next.js level too
        }
      );
...
export async function fetchWeatherForAllCitiesServer(): Promise<City[]> {
  ...
  console.log(`📊 Cache stats: ${cacheStats.valid} valid, ${cacheStats.expired} expired, ${cacheStats.total} total`);
  ...
  console.log(`✅ Cache hits: ${cachedResults.length}, API calls needed: ${uncachedCities.length}`);
  ...
  if (uncachedCities.length > 0) {
    ...
    for (let i = 0; i < uncachedCities.length; i += BATCH_SIZE) {
      ...
      console.log(`🔄 Processing batch ${Math.floor(i/BATCH_SIZE) + 1} of ${Math.ceil(uncachedCities.length/BATCH_SIZE)}`);
      
      // Process batch with delays
      const batchPromises = batch.map((city, index) => 
...
export async function getCacheStatsServer() {
  ...
  const cacheHitRate = totalCities > 0 ? Math.round((stats.valid / totalCities) * 100) : 0;
